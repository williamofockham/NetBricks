...
void* thr(void* arg) {
    ...
    while (1) {
        ...
        if (i != 0) {
            ...
            for (i = 0; i < 32; i++) {
                struct ether_hdr* hdr     = rte_pktmbuf_mtod(pkts[i], struct ether_hdr*);
            ...
            sent += send;
            if (cursec() != start_sec) {
                printf("%d %d rx=%d tx=%d\n", n->core, (cursec() - start_sec), rcvd, sent);
                /*printf("recv_pkt\n");*/
...
int main(int argc, char* argv[]) {
    ...
    /*}*/
    enumerate_pmd_ports();
    ret = init_pmd_port(PORT_OUT, THREADS, THREADS, rxq_cores, txq_cores, 256, 256, PORT_OUT == PORT_IN, 0, 0);
    assert(ret == 0);
    if (PORT_IN != PORT_OUT) {
