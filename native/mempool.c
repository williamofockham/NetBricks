...
_Static_assert(METADATA_SLOT_SIZE % RTE_MBUF_PRIV_ALIGN == 0,
               "Must be aligned to RTE_MBUF_PRIV_ALIGN");
...
int init_mempool_core(int core) {
    ...
    pframe_pool[core] = rte_pktmbuf_pool_create(name, core_mempool_size, core_mempool_cache_size,
...
#else
        mbuf             = rte_pktmbuf_alloc(pframe_pool[sid]);
        if (mbuf == NULL || mbuf->next != NULL || mbuf->pool == NULL) {
            rte_panic("Bad mbuf");
        }
        mbuf_template[sid] = *mbuf;
        rte_pktmbuf_free(mbuf);
...
int mbuf_free_bulk(mbuf_array_t array, int cnt) {
    ...
    if (i < cnt) {
        ...
        if (unlikely(mbuf->pool != _pool || mbuf->next != NULL || rte_mbuf_refcnt_read(mbuf) != 1 ||
