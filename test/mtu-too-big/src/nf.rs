...
pub fn nf<T: 'static + Batch<Header = NullHeader>, S: Scheduler + Sized>(
    ...
    let mut groups = parent.parse::<MacHeader>().group_by(
        2,
        box |pkt| match pkt.get_payload().len() as u32 > IPV6_MIN_MTU - (MacHeader::size() as u32) {
            true => {
...
fn send_too_big<T: 'static + Batch<Header = MacHeader>>(parent: T) -> CompositionBatch {
    parent
        ...
        .parse::<Ipv6Header>()
        .transform(box |pkt| {
            assert_eq!(pkt.get_header().payload_len() as usize, pkt.get_payload().len());
        ...
        .transform(box |pkt| {
            ...
            pkt.trim_payload_size(
                payload_len - ((IPV6_TOO_BIG_PAYLOAD_LEN as usize) - <Icmpv6PktTooBig<Ipv6Header>>::size())
            );
